...
@section('title','Reporte: Secciones')

@section('content_header')
  <h1 class="text-center w-100">Reporte · Secciones</h1>
@endsection
...
<div class="container-xl">
  <div class="card card-outline card-primary">
    <div class="card-header d-flex justify-content-between align-items-center">
      <h3 class="card-title mb-0">Agregados por sección/municipio</h3>
      <div class="btn-group">
        <a href="{{ route('reportes.secciones.export.xlsx', request()->query()) }}" class="btn btn-success btn-sm">
...
<script>
(function(){
  const dataUrl = @json(route('reportes.secciones.data', request()->query()));
  ...
  fetch(dataUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
    ...
    .then(json => {
      ...
      $tbody.innerHTML = rows.map(r => {
        ...
        return `<tr>
          <td>${n}</td>
          <td>${esc(fmt(r.municipio))}</td>
          <td>${esc(fmt(r.cve_mun))}</td>
          <td>${esc(fmt(r.seccion))}</td>
